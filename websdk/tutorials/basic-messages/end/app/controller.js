...
(function() {
  ...
  layerSampleApp.Controller = function(client) {
    ...
    function initializeQueries() {
      ...
      /**
       * Any time a Message in the selected Conversation is created, deleted, or has its
       * recipientStatus property change, rerender the message list.
       */
    ...
    function selectConversation(conversationId) {
      ...
      // This change will trigger the above messagesQuery "change" event handler
      // which will rerender the messageListView.
      messagesQuery.update({
        predicate: 'conversation.id = "' + conversationId + '"'
      });

      // Clear any text left over in the Message Composer
      messageComposerView.clear();
    }

    /**
     * Handle the user requesting a message be sent.
     */
