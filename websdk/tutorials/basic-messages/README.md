...
Recall that the `views/message-composer.js` file renders a text box; typing in text and hitting ENTER triggers a `message:new` event, which in turn triggers the `controller.js` `sendMessage` function.  Lets implement that method.  Open up `controller.js` and update `sendMessage`:
...
render: function(messages) {
    // Do NOT modify the query data; use concat to create a copy of the array before
    // reversing the order. Reverse the order so that most recent message is at the bottom
    // of our View
...
So, if this is called with an array of Messages it will call `addMessage` on each Message.
The `addMessage` method will create a Message View and append it to the dom.

Next, open your `controller.js` file and setup the Query in the `initializeQueries` function:

```javascript
messagesQuery = client.createQuery({
...
Open up your `views/conversatoin-list.js` file and replace the `render` method with:
...
## Step 4: Working with Read Receipts
